        ...
        <div class="footersocial">
            @if ($permission['social_link'] == 1)
                ...
                <div class="inlineEl pt20">
                    <div>
                        <div class="joinbx">
                            <a href="@if (isset($Facebook['url'])) {{ $Facebook['url'] }} @endif" target="_blank"
                                rel="noopener" class="nonecolor circle-icon fbook">
                                <i class="ion-social-facebook"></i>
                            </a>
                            <a href="@if (isset($GooglePlus['url'])) {{ $GooglePlus['url'] }} @endif" target="_blank"
                                rel="noopener" class="nonecolor circle-icon instagram">
                                <i class="ion-social-instagram"></i>
                            </a>
                            <a href="@if (isset($Twitter['url'])) {{ $Twitter['url'] }} @endif" target="_blank"
                                rel="noopener" class="nonecolor circle-icon snapchat  small"><i
                                    class="ion-social-snapchat"></i>
                            </a>
                            <a href="@if (isset($Linkedin['url'])) {{ $Linkedin['url'] }} @endif" target="_blank"
                                rel="noopener" class="nonecolor circle-icon twitter"><i class="ion-social-twitter"></i>
                            </a>
                            <a href="@if (isset($Dribbble['url'])) {{ $Dribbble['url'] }} @endif" target="_blank"
                                rel="noopener" class="nonecolor circle-icon twitter"><i class="ion-social-twitter"></i>
                            </a>
                        </div>
                    </div>
                            ...
                            <div class="payment-section-img">
                                ...
                                <ul>
                                    @foreach ($services as $service)
                                        <li><a href="#"><img
                                                    src="{{ asset('service/image') }}/{{ $service['image'] }}"
                                                    alt=""></a></li>
                                    @endforeach
                                </ul>
                            </div>
...
{{-- Update cart item in header cart --}}
<script>
    $(document).on('click', '.btnItemUpdate', function() {
        ...
        $.ajax({
            ...
            url: '/update-cart-item-frm-header',
            type: 'post',
            success: function(resp) {
                // alert(resp.totalCartItem);
                if (resp.status == false) {
                    alert(resp['message']);
                }
                $("#UpdateCartItem").html(resp.view);
                $("#cart-item-header").html(resp.totalCartItem);
            },
            error: function() {
                alert('Error');
            },
...
{{-- Remove cart item from header cart --}}
<script>
    $(document).on('click', '.removeItem', function() {
        ...
        $.ajax({
            ...
            type: 'get',
            url: '/delete-cart-item-frm-header',
            dataType: "json",
            data: {
                "cartid": cartid
            },
